...
1. Begin by making a single directory titled `Lucky_Duck_Investigations`. `mkdir Lucky_Duck_Investigations`

2. In this directory, create a directory for this specific investigation titled `mkdir Lucky_Duck_Investigations/Roulette_Loss_Investigation`. 

3. In `Roulette_Loss_Investigation`, create the following directories:

    - `Player_Analysis` to investigate the casino player. `mkdir Lucky_Duck_Investigations/Roulette_Loss_Investigations/Player_Analysis` 
    - `Dealer_Analysis` to investigate the dealers. `mkdir  Lucky_Duck_Investigations/Roulette_Loss_Investigations/Dealer_Analysis`
    - `Player_Dealer_Correlation` to summarize your findings of the collusion. `mkdir  Lucky_Duck_Investigations/Roulette_Loss_Investigations/Player_Dealer_Correlation`
...
Complete the dealer analysis. 
  ...
  Solution Script: `cat DEALER_SCHEDULE_FINAL | awk -F " " '{print $1, $2, $3, $4, $5, $6}' | grep "$2"`
...
- In case there is future fraud on the other Lucky Duck games, create a shell script called `roulette_dealer_finder_by_time_and_game.sh` that has the three following arguments:
  ...
  **Hint:** The argument does not need to name the specific casino game.
   ...
   Solution Script `cat DEALER_SCHEDULE | awk -F" " '{print $1, $2, '$3', '$4'}' | grep "$2"`
